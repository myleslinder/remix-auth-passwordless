...
type CodeErrorType = Exclude<MagicLinkErrorType, "mismatch">;

type AuthTypeErrors = {
	link: Partial<Record<MagicLinkErrorType, string>>;
...
type BaseStrategyOptions<User> = {
	...
	/**
	 * How long the magic link and code will be valid. Default to 5 minutes.
	 * @default 30_000
	 */
...
type LinkStrategyOptions = {
	...
	errorMessages?: Omit<Partial<AuthTypeErrors>, "code">;
};
...
type CodeStrategyOptions = {
	 ...
	 * @default size=8,segmentLength=4,lettersOnly=false
	 */
...
type EmailLinkStrategyOptions<User> = BaseStrategyOptions<User> &
...
type VerifyEmailFunction = NonNullable<
	EmailLinkStrategyOptions<unknown>["verifyEmailAddress"]
>;
